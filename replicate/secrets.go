...
func (*secretActions) install(r *replicatorProps, meta *metav1.ObjectMeta, sourceObject interface{}, fromObject interface{}) error {
	sourceSecret := sourceObject.(*v1.Secret)
	...
	if fromObject != nil {
		fromSecret := fromObject.(*v1.Secret)

		if fromSecret.Data != nil {
			secret.Data = make(map[string][]byte)
			for key, value := range fromSecret.Data {
				newValue := make([]byte, len(value))
				copy(newValue, value)
				secret.Data[key] = newValue
			}
